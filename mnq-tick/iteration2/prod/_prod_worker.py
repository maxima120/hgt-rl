...
FRAME = 6
STAGE0_TAG = 'NOTICK-930-12am'

MODEL_PATH = f"../stage-5/model_{STAGE0_TAG}_{FRAME}s.joblib"
PRED_PATH = f"../stage-5/lgbm5b_{STAGE0_TAG}_{FRAME}s.pqt"     # replay reference
...
ZMQ_IN = "tcp://*:50120"              # SC PUSH -> worker PULL
ZMQ_OUT = "tcp://*:50220"             # worker PUSH -> SC PULL
...
CLASSES = [("MNQ_D1", "opp"), ("MNQ_D1", "conf"), ("MNQ_D2", "opp"), ("MNQ_D2", "conf"),
           ("TICK_D1", "opp"), ("TICK_D1", "conf"), ("TICK_D2", "opp"), ("TICK_D2", "conf"),
           ("MNQ_JMA_SELF", "all")]
OSC_STREAMS = [("MNQ_D1", "jmaD1"), ("MNQ_D2", "jmaD2"), ("TICK_D1", "tickJmaD1"), ("TICK_D2", "tickJmaD2")]
VALUE_BASE = ["z_jmaD1_signed", "z_jmaD2_signed", "z_tickJmaD1_signed", "z_tickJmaD2_signed", "z_jmaD1_mag", "z_jmaD2_mag", "z_tickJmaD1_mag", "z_tickJmaD2_mag", "z_leg_amp", "z_body_raw_signed", "z_body_raw_mag"]

IN_MSG = struct.Struct("<B7f")        # flag(1 = first bar of session), 7 floats
OUT_MSG = struct.Struct("<iffii")     # t_local echo, p, p_cal, lamp, dwell
...
class Engine:
    ...
    def ingest(self, rawOpen, rawLast, jma, d1, d2, td1, td2):        
        self.k += 1
        k = self.k
        ...
        feats[col] = min(t * FRAME // (TOD_BIN_MIN * 60), N_TOD - 1)      # W.10
...
def serve(scorer):
    ...
    dwell = 0
    while True:
        flag, rawOpen, rawLast, jma, d1, d2, td1, td2 = IN_MSG.unpack(pull.recv())
        if flag or eng is None:                                            # W.7
            eng = Engine()
            dwell = 0
        t, feats = eng.ingest(rawOpen, rawLast, jma, d1, d2, td1, td2)

        if feats is None:
            dwell = 0
            push.send(OUT_MSG.pack(t, -1.0, -1.0, 0, 0))
            continue
            
        p, p_cal = scorer.predict(feats.reshape(1, -1))
        lamp = lamp_of(float(p_cal[0]))
        dwell = dwell + 1 if lamp == 3 else 0
        push.send(OUT_MSG.pack(t, float(p[0]), float(p_cal[0]), lamp, dwell))
